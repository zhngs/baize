...
runtime::EventLoop::EventLoop()
    : epollfd_(::epoll_create1(EPOLL_CLOEXEC)), events_(kepollEventSize),
...
void runtime::EventLoop::start() {
  ...
  while (1) {
    ...
    int numEvents = ::epoll_wait(epollfd_, &*events_.begin(),
                                 static_cast<int>(events_.size()), epolltime);
...
time::TimerId runtime::EventLoop::runAfter(double delay,
                                           time::TimerCallback cb) {
