    ...
    while (1) {
        ...
        if (numEvents > 0) {
            ...
            for (int i = 0; i < numEvents; i++) {
                ...
                LOG_TRACE << "fd " << fd << " epoll event is [" << getEpollEventString(events) << "]";
...
void runtime::EventLoop::schedule(WaitRequest req)
{
    if (waitRequests_.find(req) != waitRequests_.end()) {
        ...
        LOG_TRACE << "erase wait request { {fd:" << req.first
                  << ", mode:" << (req.second == WAIT_READ_REQUEST ? "WAIT_READ_REQUEST" : "WAIT_WRITE_REQUEST")
                  << "} : routine" << id << " }";
...
void runtime::EventLoop::backToMainRoutine()
{
    Routine::hangup();
}
...
void runtime::EventLoop::addWaitRequest(int fd, int mode, uint64_t routineid)
{
    ...
    LOG_TRACE << "add wait request { {fd:" << fd
