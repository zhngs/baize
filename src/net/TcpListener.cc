...
net::TcpListener::TcpListener(uint16_t port)
    : started_(false), loop_(runtime::getCurrentLoop()), listenaddr_(port),
      sock_(
          std::make_unique<Socket>(creatTcpSocket((listenaddr_.getFamily())))) {
  ...
  while (1) {
    ...
    if (connfd < 0) {
      if (errno == EAGAIN || errno == EINTR) {
        loop_->addWaitRequest(sock_->getSockfd(), WAIT_READ_REQUEST,
