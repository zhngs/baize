...
void server_print() {
  ...
  LOG_INFO << "discard server read speed " << speed << " MiB/s, " << g_read_msg
...
void client_print() {
  ...
  LOG_INFO << "discard client write speed " << speed << " MiB/s, " << g_send_msg
           << " Msg/s, " << bytes_msg << " bytes/msg";
...
void discard_client() {
  ...
  while (1) {
    int wn = stream->asyncSendto(message.c_str(),
                                 static_cast<int>(message.size()), serveraddr);
