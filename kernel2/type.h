...
public:
    ...
    string dump(string prefix, int linebytes = 8) {
        string s;
        char buf[128] = "";
        snprintf(buf, sizeof(buf), "[%s] slice stat: ptr=%d len=%d cap=%d begin=%d end=%d\n",
            prefix.c_str() , ptr_->data(), this->len(), this->cap(), begin_, end_);
        s += buf;
        ...
        for (int i = 0; i <= end_pos; ) {
            ...
            for (int j = 0; j < linebytes; j++) {
                if (i + j <= end_pos) {
                    if ((j + 1) % 8 == 0) {
                        snprintf(buf + cur_pos_in_line, sizeof(buf) - cur_pos_in_line, "%02x  ", self[i + j]);
                        cur_pos_in_line += 4;
                    } else {
                        snprintf(buf + cur_pos_in_line, sizeof(buf) - cur_pos_in_line, "%02x ", self[i + j]);
                        cur_pos_in_line += 3;
            ...
            for (int j = 0; j < linebytes; j++) {
                if (i + j <= end_pos) {
                    snprintf(buf + cur_pos_in_line, sizeof(buf) - cur_pos_in_line, "%c", printchar(self[i + j]));
                } else {
                    snprintf(buf + cur_pos_in_line, sizeof(buf) - cur_pos_in_line, ".");
                }
                cur_pos_in_line++;
