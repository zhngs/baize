...
int SdpParse(StringPiece message, SdpMessage& sdp)
{
    ...
    if (!ice_ufrag.empty()) {
        sdp.net_.ice_ufrag_ = ice_ufrag.AsString();
    }

    StringPiece ice_pwd = message.SliceFragment("ice-pwd:", "\r\n");
    if (!ice_pwd.empty()) {
        sdp.net_.ice_pwd_ = ice_pwd.AsString();
    }

    StringPiece finger_print = message.SliceFragment("fingerprint:", "\r\n");
    if (!finger_print.empty()) {
        sdp.net_.finger_print_ = finger_print.AsString();
    }

    LOG_INFO << "ice-ufrag:" << sdp.net_.ice_ufrag_;
    LOG_INFO << "ice-pwd:" << sdp.net_.ice_pwd_;
    LOG_INFO << "fingerprint:" << sdp.net_.finger_print_;

    StringPiece cname = message.SliceFragment("cname:", "\r\n");
    if (!cname.empty()) {
        sdp.cname_ = cname.AsString();
    }

    LOG_INFO << "cname:" << sdp.cname_;
    ...
    for (auto& media_item : media_vector) {
        ...
        sdp.tracks_.push_back(std::move(track));
    }

    for (auto& track : sdp.tracks_) {
