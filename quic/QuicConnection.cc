...
net::QuicConnection::QuicConnection(UdpStreamSptr udpstream,
                                    ...
                                    QuicConfigSptr config, quiche_conn *conn)
    : udpstream_(udpstream), localaddr_(localaddr), peeraddr_(peeraddr),
...
net::QuicConnSptr net::QuicConnection::connect(const char *ip, uint16_t port) {
  ...
  quiche_conn *conn = quiche_connect(
      ip, scid, sizeof(scid), localaddr.getSockAddr(), localaddr.getSockLen(),
...
bool net::QuicConnection::flushQuic() {
  ...
  while (1) {
    ssize_t written = quiche_conn_send(conn_, quicWriteBuffer,
                                       sizeof(quicWriteBuffer), &send_info);
...
bool net::QuicConnection::untilEstablished() {
  while (1) {
    ...
    ssize_t read = udpstream_->asyncRecvfrom(quicReadBuffer,
                                             sizeof(quicReadBuffer), &peeraddr);
    if (read < 0) {
      return false;
    }
...
int net::QuicConnection::quicStreamWrite(uint64_t streamid, const void *buf,
                                         int len, bool fin) {
...
int net::QuicConnection::quicStreamRead(uint64_t streamid, void *buf, int len,
                                        bool *fin) {
  ssize_t rn = quiche_conn_stream_recv(conn_, streamid,
                                       static_cast<uint8_t *>(buf), len, fin);
  return static_cast<int>(rn);
}

void net::QuicConnection::quicConnRead(void *buf, int len,
                                       InetAddress &peeraddr) {
  quiche_recv_info recv_info = {
      peeraddr.getSockAddr(),
      peeraddr.getSockLen(),
  ...
  if (quiche_conn_is_established(conn_)) {
    ...
    while (quiche_stream_iter_next(readable, &s)) {
      ...
      bool fin = false;
      ssize_t recv_len = quiche_conn_stream_recv(conn_, s, quicReadBuffer,
                                                 sizeof(quicReadBuffer), &fin);
      if (recv_len < 0) {
        break;
      }
