...
void net::QuicListener::loopAndAccept()
{
    while (1) {
        ...
        int rc = quiche_header_info(quicReadBuffer,
                                    rn,
                                    kConnIdLen,
                                    &data.version,
                                    &data.type,
...
bool net::QuicListener::quicNegotiate(QuicListenerData* data)
{
    ...
    ssize_t written = quiche_negotiate_version(data->scid,
                                               data->scid_len,
                                               data->dcid,
                                               data->dcid_len,
                                               quicWriteBuffer,
...
static bool validate_token(const uint8_t* token,
                           size_t token_len,
                           struct sockaddr* addr,
                           socklen_t addr_len,
                           uint8_t* odcid,
...
bool net::QuicListener::validateToken(QuicListenerData* data)
{
    ...
    if (data->token_len == 0) {
        ...
        bool ret =
            RandomFile::getInstance().genRandom(new_cid, sizeof(new_cid));
        if (!ret) {
            return false;
        }

        ssize_t written = quiche_retry(data->scid,
                                       data->scid_len,
                                       data->dcid,
                                       data->dcid_len,
                                       new_cid,
...
net::QuicConnSptr net::QuicListener::quicAccept(QuicListenerData* data)
{
    ...
    quiche_conn* conn = quiche_accept(data->dcid,
                                      data->dcid_len,
                                      data->odcid,
                                      data->odcid_len,
                                      localaddr_.sockaddr(),
