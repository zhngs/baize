...
struct net::QuicListenerData {
  QuicListenerData()
      : type(0), version(0), scid(), scid_len(sizeof(scid)), dcid(),
        dcid_len(sizeof(dcid)), odcid(), odcid_len(sizeof(odcid)), token(),
...
void net::QuicListener::loopAndAccept() {
  while (1) {
    ...
    int rn = udpstream_->asyncRecvfrom(quicReadBuffer, sizeof(quicReadBuffer),
                                       &data.peeraddr);
    if (rn < 0) {
      continue;
    }
    ...
    int rc = quiche_header_info(
        quicReadBuffer, rn, kConnIdLen, &data.version, &data.type, data.scid,
        &data.scid_len, data.dcid, &data.dcid_len, data.token, &data.token_len);
    if (rc < 0) {
...
bool net::QuicListener::quicNegotiate(QuicListenerData *data) {
  ...
  ssize_t written = quiche_negotiate_version(
      data->scid, data->scid_len, data->dcid, data->dcid_len, quicWriteBuffer,
      sizeof(quicWriteBuffer));
  if (written < 0) {
    LOG_ERROR << "failed to create vneg packet: " << written;
    return false;
  }

  int sent = udpstream_->asyncSendto(quicWriteBuffer, static_cast<int>(written),
                                     data->peeraddr);
  if (sent != written) {
...
bool net::QuicListener::validateToken(QuicListenerData *data) {
  ...
  if (data->token_len == 0) {
    mint_token(data->dcid, data->dcid_len, data->peeraddr.getSockAddr(),
               data->peeraddr.getSockLen(), data->token, &data->token_len);
    ...
    ssize_t written =
        quiche_retry(data->scid, data->scid_len, data->dcid, data->dcid_len,
                     new_cid, sizeof(new_cid), data->token, data->token_len,
...
net::QuicConnSptr net::QuicListener::quicAccept(QuicListenerData *data) {
  ...
  quiche_conn *conn =
      quiche_accept(data->dcid, data->dcid_len, data->odcid, data->odcid_len,
                    localaddr_.getSockAddr(), localaddr_.getSockLen(),
                    data->peeraddr.getSockAddr(), data->peeraddr.getSockLen(),
                    config_->getConfig());
  if (conn == nullptr) {
    return QuicConnSptr();
  ...
  return QuicConnSptr(std::make_shared<QuicConnection>(
      udpstream_, localaddr_, data->peeraddr, config_, conn));
}